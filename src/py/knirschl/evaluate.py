...
def global_compare(root_output, replicates, tag):
    ...
    with open(os.path.join(fam.get_metrics_dir(root_output), "DL_global__rf_distance_avg-rel.txt"), 'r') as reader:
        line = reader.readline()
        if (line != ''):
            split = line.split(" : ")
    ...
    with open(os.path.join(root_output, "metrics", "generax_picks.txt"), 'r') as reader:
        ...
        for line in reader.readlines():
            ...
            try:
                ...
                pass
            print(replicate, family, tree_id, "Stats picked:", stats_picked)
            print(replicate, family, best_avg_tree, "Stats best on average:", stats_best_avg)
