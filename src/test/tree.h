//
// Created by knirschl on 06.07.23. TODO copy to nj/tree @02.05.
//
...
public:
    ...
    int reroot(int root_child) {
        ...
        if (overwrite_left) {
            make_node(new_root, root_child, old_parent);
        } else {
            make_node(new_root, old_parent, root_child);
        }
    ...
    */
    ...
    std::string node_info(int cur) {
        if (tree[cur].is_leaf) {
            return std::format("LEAF {}[par= {}, dup= {}]", idx2leafname.at(cur),
                               get_name_or_idx(tree[cur].parent_idx), tree[cur].is_dup);
        }
